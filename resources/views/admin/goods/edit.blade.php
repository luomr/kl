            ...
            <div class="col-sm-8">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form enctype="multipart/form-data" method="post" action="/admin/goods/{{$result->id}}" class="form-horizontal m-t" id="commentForm">
                            <?php echo method_field('PUT'); ?>
                            <?php echo csrf_field(); ?>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">商品名称：</label>
                                <div class="col-sm-8">
                                    <input id="cname" name="listname" minlength="2" type="text" class="form-control" required="" aria-required="true" value="{{ $result->listname }}">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <option value="{{$type1->id}}">{{$type1->catename}}</option>
                                    @foreach($data as $v)
                                    @if($v->catepid != 0)
                                    <option value="{{$v->id}}">
                                     @if($v->level!==0)|@endif
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <textarea id="cname" name="listtitle" class="form-control" required="" aria-required="true" >{{$result->listtitle}}</textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select class="form-control m-b" name="status" required="" aria-required="true">
                                        <option value=1 @if($result->status==1) selected @endif >上架</option>
                                        <option value=2 @if($result->status==2) selected @endif>下架</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
