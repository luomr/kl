            ...
            <div class="col-sm-8">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form enctype="multipart/form-data" method="post" action="/admin/admin/{{ $result->id}}" class="form-horizontal m-t" id="commentForm">
                            {{ method_field('PUT') }}
                            {!! csrf_field() !!}
                            <div class="form-group">
                                <label class="col-sm-3 control-label">名称：</label>
                                <div class="col-sm-8">
                                    <input id="cname" name="name" minlength="2" type="text" class="form-control" required="" aria-required="true" value="{{ $result->name}}">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">头像：</label>
                                <div class="col-sm-8">
                                    <input name="thumb" type="file" value="" style="float:left">
                                    <img src="{{ $result->thumb}}" alt="" style="margin-left:-60px;width:30px">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">密码：</label>
                                <div class="col-sm-8">
                                    <input id="cemail" type="password" class="form-control" name="password" required="" aria-required="true" value="{{ $result->password}}">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">权限：</label>
                                <div class="col-sm-4">
                                    <select class="form-control m-b" name="role" required="" aria-required="true">
                                        <option @if($result->role=='超级管理员') selected @endif>超级管理员</option>
                                        <option @if($result->role=='普通管理员') selected @endif>普通管理员</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
