            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>评论管理</h5>
                        <form  method="post" action="/admin/comment/search" class="pull-right mail-search" style="margin-top:-6px">
                         {!! csrf_field() !!}
                    ...
                    <div class="ibox-content">
                        <table class="table">
                            ...
                            @foreach ($stu as $v)
                                <tr>
                                    ...
                                    <td>{{$v->comshop}}</td>
                                    <td>{{$v->comname}}</td>
                                    <td><img src="{{$v->comimg}}" alt="" style="width:50px"></td>
                                    <td>{{$v->comcontent}}</td>
                                    <td>{{$v->comdata}}</td>
                                    <td>{{$v->comback}}</td>
                                    <td><a href="javascript:;" onclick='changeState(this,{{$v->id}},"{{$v->comstate}}")' title="{{$v->comstate}}">{{$v->comstate}}</a></td>
                                    <td><a href="/admin/comment/{{$v->id}}/edit">回复评论</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                        ...
                        <div id="page">
                            {{ isset($keywords) ? $stu->appends(['keywords' => "$keywords"])->links() : $stu->links() }}
                        </div>
                    </div>
                </div>
            </div>
